...
func init() {
	formats.Register("act.v1", New())
}

type AdapterACT struct{}

func New() *AdapterACT { return &AdapterACT{} }

func (a *AdapterACT) Import(ctx context.Context, r io.Reader) (formats.ExamLike, formats.Policy, error) {
	_, _ = ioutil.ReadAll(r)
	return nil, formats.Policy{}, errors.New("act.v1 import not implemented")
}

func (a *AdapterACT) Export(ctx context.Context, ex formats.ExamLike, pol formats.Policy) (io.ReadCloser, error) {
	// Placeholder
	return io.NopCloser(stringsNewReader(
		fmt.Sprintf("ACT export placeholder\nexam=%s title=%s\n", ex.GetID(), ex.GetTitle()),
	)), nil
}

func (a *AdapterACT) Validate(ex formats.ExamLike, pol formats.Policy) error {
	if err := formats.ValidatePolicy("act.v1", &pol); err != nil {
		return err
	}
	...
	for _, q := range ex.GetQuestions() {
		if q.GetType() == "mcq_single" && len(q.GetChoices()) > 0 && len(q.GetChoices()) != 4 {
			return fmt.Errorf("act.v1: question %s must have exactly 4 choices", q.GetID())
		}
		// No SPR typically; if you use "numeric" as SPR, discourage here.
		if q.GetType() == "numeric" && len(q.GetChoices()) == 0 {
			return fmt.Errorf("act.v1: numeric SPR-style items not supported in MVP")
		}
	}
